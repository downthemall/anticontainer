...
/* globals DTA, Utils, QueueItem, RequestManipulation, DTA_URL, Dialog */
/* globals UrlManager, Preferences, startDownloads, Tree, RUNNING */
/* globals Cc, Ci, escape, alert */
/* globals log, LOG_DEBUG, LOG_ERROR */
...
const _ = Cu.import("chrome://dtaac-modules/content/l10n.jsm", {}).
...
const {decodeEntities} =
  Cu.import("chrome://dtaac-modules/content/entities.jsm", {});
const {privatizeXHR} =
  Cu.import("chrome://dtaac-modules/content/utils.jsm", {});
...
Cu.import(
  "chrome://dtaac-modules/content/sandboxfactories.jsm", _sandboxFactories);
...
acResolver.prototype = {
  ...
  run: function acR_run(download) {
    ...
    if (this.download._acAttempt >= 3 ||
        this.download._acGlobalAttempt >= 20) {
  ...
  process: function DR_process() {
    ...
    this.download.status =
      _('acStatus', [this.prefix, this.download._acAttempt - 1]);
    ...
    // We are not a third party, but this will give us cookies as a
    // primary party
    if (this.req.channel &&
        (this.req.channel instanceof Ci.nsIHttpChannelInternal)) {
  ...
  setURL: function acR_setURL(url, nameSuggestion) {
    ...
    if (!this.download._acOriginal &&
        this.type != 'redirector' && !this.static) {
    ...
    var nu = this.composeURL(
      this.req ?
      this.req.channel.URI.spec :
      this.download.urlManager.url.spec,
      urld
    );
    // set full path
    url = nu.url;
    ...
    let useServerName = (this.useServerName && !this.useOriginName) ||
                        this.type == 'redirector';
    if (!dn) {
      dn = Utils.getUsableFileName(this.download.urlManager.usable);
      useServerName = true;
    }
  ...
  finish: function acR_finish() {
    ...
    if (this.addedDownloads && this.addedDownloads.length > 0) {
      if (this.addedDownloads.length === 0) {
        this.setURL(
          this.addedDownloads[0].url, this.addedDownloads[0].nameSuggestion);
      }
      else {
        ...
        const newNumForSpawned =
          Preferences.getExt("anticontainer.newnumforspawned", false);

        (function spawnCtor() {
          function SpawnedQueueItem(inst, item) {
            let nu = inst.composeURL(
              inst.req ?
              inst.req.channel.URI.spec :
              inst.download.urlManager.url.spec,
            ...
            if (!!item.nameSuggestion && !!item.nameSuggestion.trim()) {
              this.destinationName = this.fileName =
                makeFileName(item.nameSuggestion);
            }
          }
          SpawnedQueueItem.prototype = {
            ...
            numIstance: newNumForSpawned ?
              series.num :
              (this.download.numInstance || this.download.bNum),
          ...
          this.addedDownloads = this.addedDownloads.map(
            e => new SpawnedQueueItem(this, e));
        }).call(this);

        // add new downloads
        startDownloads(
          true /* since this download was running already */,
          this.addedDownloads,
          false);
        this.removeDownload = true;
      }
  ...
  _createSandboxInternal: function acR_createSandboxInternal() {
    let tp = this;
    let sb = this._sb = Components.utils.Sandbox(
      this.download.urlManager.url.spec,
      {
    ...
    try {
      ...
      Components.utils.evalInSandbox(
        this.getSandboxScripts(),
        sb,
        "latest",
        "chrome://dtaac/content/sandboxscripts.js",
        1);
    }
    catch (ex) {
  ...
  generateResolve: function aCR_generateResolve(obj) {
    ...
    return function() {
      if (this.status >= 400 &&
          [401, 402, 407, 500, 502, 503, 504].indexOf(this.status) != -1) {
      ...
      if (m) {
        try {
          let name = null;
          let u = this.generateReplacement(
            obj.builder,
            m,
            this.download.urlManager.url.spec.match(obj.match));
          if ("namer" in obj) {
            name = this.generateReplacement(
              obj.namer,
              m,
              this.download.urlManager.url.spec.match(obj.match));
          }
  ...
  generateSandboxed: function aCR_generateSandboxed(fn, fnname) {
    return function() {
      ...
      try {
        ...
        // This isn't new code, but around for years in AMO-approved versions
        // of this add-on.
        //
        // One one hand, such plugin scripts are pretty limited as the above
        // functions detail. One the other hand, they have powerful XHR
        // (but sans cookie-reading and direct DOM access; just "string"
        // access).
        // All-in-all they might be (almost) as dangerous as Greasemonkey
        // user-scripts. As with GM user-scripts the user has to manually
        // install new plugins.
        Components.utils.evalInSandbox(
          fn,
          sb,
          "latest",
          `anticontainer-${this.prefix}-${fnname}`,
          1);
      }
      catch (ex) {
        log(LOG_ERROR,
            "Failed to create or execute sandboxed plugin " + this.prefix,
            ex);
        sb._finish();
      }
    };
  ...
  generateExpanded: function aCR_generateExpanded(obj) {
    ...
    return function() {
      ...
      if (m) {
        ...
        while ((m = obj.finder.exec(this.responseText)));
...
Cu.import(
  'chrome://dtaac-modules/content/replacementgenerator.jsm',
  acResolver.prototype);
Cu.import(
  'chrome://dtaac-modules/content/urlcomposer.jsm', acResolver.prototype);
Cu.import(
  'chrome://dtaac-modules/content/sandboxscripts.jsm', acResolver.prototype);
...
function acFactory(obj) {
  ...
  case 'redirector':
    this.obj.prototype.process = function() {
      let nu =
        this.download.urlManager.url.spec.replace(obj.pattern, obj.replacement);
      this.setURL(nu);
      this.finish();
    };
    break;
  ...
  case 'sandbox':
    if (obj.process) {
      this.obj.prototype.process =
        acResolver.prototype.generateSandboxed(obj.process, "process");
    }
    this.obj.prototype.defaultResolve =
      acResolver.prototype.generateResolve(obj);
    if (obj.resolve) {
      this.obj.prototype.resolve =
        acResolver.prototype.generateSandboxed(obj.resolve, "resolve");
    }
    else {
...
acFactoryManager.prototype = {
  _init: function() {
    ...
    Cc['@mozilla.org/observer-service;1'].
      ...
      addObserver(this, acPlugins.TOPIC_PLUGINSCHANGED, true);
  },
  observe: function() {
    this._reload();
  },
